...
const directory = `${process.env.EVIDENCE_DIR || 'output/playwright/v2'}/functional`
...
try {
  ...
  await page.getByRole('button',{name:'Descubrir la constelación de Esmeralda Flores'}).focus()
  await page.keyboard.press('Enter')
  ...
  check(await page.getByRole('img',{name:'Constelación Esmeralda Flores',exact:true}).isVisible(),'Nombre completo accesible mediante teclado')
  check(await page.locator('.name-word').evaluateAll(words=>words[0].getCTM().f===words[1].getCTM().f),'Nombre de escritorio compuesto en una línea')
  await shot(page,'desktop-name'); await audit(page,'desktop-name')
  for(let i=1;i<=3;i++) {
    await page.getByRole('button',{name:new RegExp(`^Descubrir mensaje en flor ${i}:`)}).click()
  ...
  await page.getByRole('button',{name:'Abrir la carta para Esmeralda Flores'}).click()
  ...
  check(await page.getByRole('button',{name:'Abrir la carta para Esmeralda Flores'}).evaluate(el=>el===document.activeElement),'Escape cierra y devuelve el foco al sobre')
  ...
  await mobile.getByRole('button',{name:'Descubrir la constelación de Esmeralda Flores'}).tap()
  ...
  check(await mobile.locator('.name-word').evaluateAll(words=>words[0].getCTM().f<words[1].getCTM().f),'Nombre móvil recompuesto en dos líneas')
  await shot(mobile,'mobile-name'); await audit(mobile,'mobile-name')
  await mobile.getByRole('button',{name:/^Descubrir mensaje en flor 1:/}).tap()
  ...
  await mobile.getByRole('button',{name:'Abrir la carta para Esmeralda Flores'}).tap(); await mobile.waitForTimeout(1000)
  ...
  await mobile.getByRole('button',{name:'Abrir la carta para Esmeralda Flores'}).tap()

import { useEffect, useReducer, useRef, useState, type CSSProperties, type MouseEvent } from 'react'
...
export function App() {
  ...
  const discover = (index: number, e: MouseEvent<HTMLButtonElement>) => {
    sparkle(e); setMessage(content.secrets[index % 3]); setMessageClosing(false)
  ...
  return <main className={`universe scene-${scene} ${motion.still ? 'still' : ''} ${motion.ambientPaused ? 'ambient-paused' : ''}`} data-scene={scene}>
    ...
    <header className="masthead">
      <div className="wordmark"><Star/><span>Un pequeño universo<span className="wordmark-sub">hecho para ti</span></span></div>
    ...
    <div className="scene-content" id="experience" tabIndex={-1} key={cycle}>
      <div className="story-copy" key={garden ? 'garden-copy' : 'main-copy'}>
        ...
        {garden ? <><h1 className="garden-title">La distancia<br/>también puede<br/><em>florecer.</em></h1><div className="garden-message"><p>{content.garden[0]}</p><p>{content.garden[1]}</p></div></> : <><h1>Una flor<br/>hasta donde<br/><em>estés.</em></h1>
        ...
        <div className="tiny-line" aria-hidden="true"/><p className="aside-note">La distancia es solo un lugar.<br/>Lo bonito siempre encuentra el camino.</p>
      </div>

      <div className="celestial-stage">
        <Observatory/>
        <span className="orbit-caption" aria-hidden="true">Donde las estrellas se vuelven flores</span>
        {scene === 'intro' && <button ref={primaryRef} className="traveler" onClick={()=>dispatch('START')} aria-label="Tocar la estrella y hacer florecer la rosa"><span className="star-halo"/><Star/><span className="star-orbit"/><span className="touch-hint">Toca la estrella<span>Hay algo para ti</span></span></button>}
        ...
        {scene !== 'intro' && <div className={`hero-flower ${burst ? 'has-sparkled' : ''}`}>
          ...
          {(scene === 'bloom' || garden) && <button ref={primaryRef} className="flower-touch" aria-label={garden ? 'Tocar los pétalos de la rosa' : 'Tocar la rosa y descubrir el jardín'} onClick={e=>{sparkle(e); if(scene === 'bloom')dispatch('PLANT'); else discover(0,e)}}><span className="touch-ring"/><span className="sr-only">Tocar pétalos</span></button>}
        ...
        {(scene === 'bloom' || scene === 'growing') && <div className="flower-caption"><span className="caption-rule"/><span>{scene === 'growing' ? 'Un poquito de luz. Un pétalo a la vez.' : 'Una rosa amarilla, solo para ti.'}</span><span className="specimen-number" aria-hidden="true">✧</span></div>}
        {scene === 'bloom' && <button className="bloom-hint" onClick={e=>{sparkle(e);dispatch('PLANT')}}>Toca la rosa. Esto apenas comienza.<span aria-hidden="true">↗</span></button>}
      </div>
      ...
      {garden && <div className="garden">
        ...
        <button className={`secret-star ${secret ? 'discovered' : ''}`} onClick={e=>{sparkle(e);setSecret(s=>!s)}} aria-label="Descubrir constelación A V" aria-pressed={secret}><Star/><span>{secret ? 'Tu constelación' : '¿Y esta estrella?'}</span></button>
        {secret && <svg className="secret-constellation" viewBox="0 0 260 110" role="img" aria-label="Constelación A, estrella, V"><path d="M20 90L55 15L90 90M36 58H74M170 15L205 90L240 15" pathLength="1"/>{[[20,90],[55,15],[90,90],[36,58],[74,58],[170,15],[205,90],[240,15]].map(([x,y])=><circle key={`${x}-${y}`} cx={x} cy={y} r="2.5"/>)}<path className="constellation-gem" d="M130 36L134 50L148 54L134 58L130 72L126 58L112 54L126 50Z"/></svg>}
        <button ref={envelopeRef} className="letter-invitation" onClick={()=>dispatch('LETTER')} aria-label="Abrir la carta para Anel"><span className="envelope-birth"><Star className="envelope-star"/><Envelope/></span><span>Hay unas palabras para ti<span>Abrir la carta</span></span></button>
        <div className={`hidden-message ${messageClosing ? 'message-closing' : ''}`} role="status" aria-live="polite">{message && <p key={message}>{message}</p>}</div>
      </div>}
    </div>

    <footer className="scene-footer"><div className="act-progress" aria-label={`Acto ${act} de 3`}><span className="act-number">0{act}<span>/ 03</span></span><span className="progress-track"><i style={{width:`${act / 3 * 100}%`}}/></span><span className="act-name">{act === 1 ? 'Una estrella de lejos' : act === 2 ? 'Un poquito de magia' : 'Un jardín para ti'}</span></div><span className="footer-note">{garden ? `${discovered.length} de 3 pequeñas sorpresas` : 'Algunas cosas bonitas no conocen distancias.'}<Star/></span></footer>
